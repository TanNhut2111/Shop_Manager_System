...
public class TaiKhoanDAO {
    ...
    public ArrayList<TaiKhoanDTO> docDB(String condition, String orderBy) throws Exception {
        ...
        ArrayList<TaiKhoanDTO> tks = new ArrayList<>();
        while ( result.next() ) {
            TaiKhoanDTO tk = new TaiKhoanDTO();
            tk.setStrTenDangNhap(result.getString("tendangnhap"));
            tk.setStrMatKhau(result.getString("matkhau"));
            tk.setiCapBac( Integer.parseInt(result.getString("capbac")) );
            tks.add(tk);
        }
        connect.Close();
        return tks;
