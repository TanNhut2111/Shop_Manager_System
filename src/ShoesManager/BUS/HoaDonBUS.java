...
public class HoaDonBUS {
    ...
    public String getNewMaHDFromDB() throws Exception {
        return hdDAO.getNewMaHD();
    }
    

    public boolean isMaKMDaSuDung(String maKM) {
        for (HoaDonDTO hd : list_HD) {
            if (hd.getDeleted() == 0 && hd.getStrMaKM() != null && hd.getStrMaKM().equals(maKM)) {
                return true; // Mã khuyến mãi đang được dùng trong hóa đơn chưa bị xóa
            }
        }
        return false; // Tất cả hóa đơn dùng mã này đều đã bị xóa mềm
    }
}
