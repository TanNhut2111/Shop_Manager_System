...
public class KhuyenMaiBUS {
    ...
    public String getDefaultMaHD() {
        if (list_KM.size() == 0)
            ...
            for (KhuyenMaiDTO km : list_KM) {
                String[] maKM = km.getStrMaKM().split("SP");
                iNumb = Integer.parseInt( maKM[1] );
                iNumb++;
            }
        ...
        if ( khDAO.xoa(kh) ) {
            ...
            for ( KhuyenMaiDTO taikhoan : list_KM ) {
                if (taikhoan.getStrMaKM().equals(kh.getStrMaKM())){
                    taikhoan.setDeleted(1); // cập nhật lại flag trong bộ nhớ
                break;
                }
            }
        }
    ...
    public String getNewMaKMFromDB() throws Exception {
        return khDAO.getNewMaKM();
    }
}
