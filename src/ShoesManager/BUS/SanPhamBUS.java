    ...
    public String getDefaultMaHD() {
        if (list_SP.size() == 0)
            ...
            for (SanPhamDTO sp : list_SP) {
                String[] maSP = sp.getStrMaGiay().split("SP");
...
//            Collections.sort(arr, SanPhamDTO::soLuongGiamdan);
    ...
    public Boolean xoa(SanPhamDTO hd) throws Exception {
        if ( spDAO.xoa(hd) ) {
            ...
            for ( SanPhamDTO sanpham : list_SP ) {
                if (sanpham.getStrMaGiay().equals(hd.getStrMaGiay())){
                    sanpham.setDeleted(1); // cập nhật lại flag trong bộ nhớ
                    return true;
                }
            }
        }
    ...
    public int getGia(String maSP) {
        for (SanPhamDTO sp : list_SP) {  // Đổi 'list_SanPham' nếu tên khác
            if (sp.getStrMaGiay().equalsIgnoreCase(maSP)) {  // Đổi 'getMaSP()' nếu tên khác
                return sp.getiGia();  // Đổi 'getGia()' nếu getter của bạn tên khác như getDonGia()
            }
        }
        return -1; // Không tìm thấy
    }

    public String getNewMaSPFromDB() throws Exception {
        return spDAO.getNewMaSP();
    } 
}
